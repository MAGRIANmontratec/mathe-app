...
const getModeInfo = (m: string): ModeInfo => {
  switch (m) {
    ...
    case "arrows": return { name: "Pfeile", icon: "➡️", color: "bg-purple-500", border: "border-purple-700" };
    ...
    case "money_compare": return { name: "Vergleich", icon: <Scale size={20} />, color: "bg-sky-500", border: "border-sky-700" };
    ...
    case "units_length": return { name: "Längen", icon: "📏", color: "bg-emerald-600", border: "border-emerald-800" };
...
const PerfectScoreAnimation = () => {
  const [effect] = useState(() => {
    const effects = ["confetti", "fireworks", "balloons", "unicorns", "stitch"];
...
export default function App() {
  ...
  const pickModeForCategory = (cat: Category): GameMode => {
    ...
    else pool = [...calcModes, ...spaceModes, ...moneyModes, ...grade3Modes];
  ...
  const generateQuestion = (targetMode: GameMode) => {
    ...
    do {
      ...
      if (targetMode === "multiplication") {
        ...
        q = {
          text: `${dividend} : ${divisor} = ?`, answer: 0,
          divisionData: { dividend, divisor, quotient, remainder },
          helpText: `Wie oft passt die ${divisor} komplett in die ${dividend}? Der Rest ist das, was übrig bleibt.`
        };
        ...
        q = {
          text: "Löse die Umkehraufgabe", answer: 0, inverseData: { a, b, add, result: res },
          helpText: "Punkt vor Strich! Rechne zuerst die Mal-Aufgabe und zähle dann die Plus-Zahl dazu."
        };
        ...
        if (hideA) {
          q = { text: <span><span className="bg-slate-200 px-3 py-1 rounded mx-1">?</span> + {b} = {sum}</span>, answer: a, helpText: `Rechne rückwärts (Umkehraufgabe): ${sum} - ${b} = ?` };
          signature = `gapA-?+${b}=${sum}`;
        } else {
          q = { text: <span>{a} + <span className="bg-slate-200 px-3 py-1 rounded mx-1">?</span> = {sum}</span>, answer: b, helpText: `Rechne rückwärts (Umkehraufgabe): ${sum} - ${a} = ?` };
          signature = `gapA-${a}+?=${sum}`;
        }
        ...
        const hideA = rand(0, 1) === 0;
        if (hideA) {
          q = { text: <span><span className="bg-slate-200 px-3 py-1 rounded mx-1">?</span> - {b} = {res}</span>, answer: a, helpText: `Rechne rückwärts (Umkehraufgabe): ${res} + ${b} = ?` };
          signature = `gapS-?-${b}=${res}`;
        } else {
          q = { text: <span>{a} - <span className="bg-slate-200 px-3 py-1 rounded mx-1">?</span> = {res}</span>, answer: b, helpText: `Wie viel fehlt von der ${res} bis zur ${a}? Rechne: ${a} - ${res} = ?` };
          signature = `gapS-${a}-?=${res}`;
        }
        ...
        q = {
          text: "Rechne schrittweise", answer: a + b,
          stepData: { start: a, operator: "+", operand: b, result: a + b, steps: [{ val: splitH, res: res1 }, { val: splitZ, res: res2 }, { val: splitE, res: res3 }] },
          helpText: "Zerlege die zweite Zahl in Hunderter, Zehner und Einer. Addiere sie Schritt für Schritt."
        };
        ...
        q = {
          text: "Rechne schrittweise", answer: a - b,
          stepData: { start: a, operator: "-", operand: b, result: a - b, steps: [{ val: splitH, res: res1 }, { val: splitZ, res: res2 }, { val: splitE, res: res3 }] },
          helpText: "Zerlege die zweite Zahl in Hunderter, Zehner und Einer. Ziehe sie nacheinander ab."
        };
        ...
        q = {
          text: "Überschlage (runde auf Hunderter)", answer: 0,
          estimationData: { a: exactA, b: exactB, roundedA, roundedB, estResult: estRes, op: isPlus ? "+" : "-" },
          helpText: "Runde beide Zahlen auf volle Hunderter. Ab 50 wird aufgerundet, darunter abgerundet. Danach rechne mit den Hundertern."
        };
        ...
        q = { text: <MoneyDisplay />, answer: totalCents, isDecimal: true, helpText: "Zähle zuerst die großen Scheine, dann die großen Münzen und zuletzt das Kleingeld." };
        ...
        q = {
          ...
          isDecimal: true,
          helpText: "Schritt 1: Rechne die Preise der beiden Dinge zusammen. Schritt 2: Ziehe die Summe von dem Geld ab, das du gibst (Rückgeld)."
        };
        signature = `shop-${p1.name}-${p2.name}`;
        setActiveCellId("shop-total");
      } else if (targetMode === "cube_buildings") {
        let rows = 3; let cols = 3;
        if (settings.cubeGridSize === "random") {
           rows = rand(2, 3); cols = rand(2, 4);
        } else {
           const parts = settings.cubeGridSize.split("x");
           rows = parseInt(parts[0]); cols = parseInt(parts[1]);
        }
        ...
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) {
            cells.push({ id: `cube-${r}-${c}`, val: g[r][c], isGiven: false });
          }
        }

        q = {
          text: <div className="flex flex-col items-center"><IsometricCubes grid={g} /></div>, answer: 0,
          gridData: { cells, type: "cube_plan", meta: { rows, cols } },
          helpText: "Zähle die Würfel. Was du nicht siehst, ist auch nicht da (schreibe 0 oder lass es leer). Jeder sichtbare Turm geht bis ganz auf den Boden."
        };
        ...
        const allShapes = [
          ...
          { id: "prism", name: "Dreiecksprisma", ecken: 6, kanten: 9, flächen: 5 }
        ];
        // Filtere nach aktiven Shapes in den Settings
        let shapes = allShapes.filter(s => settings.activeShapes.includes(s.id));
        if (shapes.length === 0) shapes = allShapes; // Fallback falls nichts markiert

        const s = shapes[rand(0, shapes.length - 1)];
        const props = ["ecken", "kanten", "flächen"] as const;
        ...
        q = { 
          text: (
              <div className="flex flex-col items-center">
                  ...
                  <div className="text-3xl sm:text-4xl font-black text-slate-800 mt-6 mb-2 text-center">{s.name}</div>
                  <div className="text-xl sm:text-2xl text-slate-600 text-center">Wie viele <span className="text-blue-600 font-bold">{capP}</span> hat dieser Körper?</div>
              </div>
          ), 
          answer: s[p],
          helpText: "Ecken sind die spitzen Punkte. Kanten sind die geraden oder runden Linien, an denen Flächen zusammenstoßen. Flächen sind die glatten Außenwände."
        };
        ...
        else if (type === 4) { const a = rand(2, 15); q = { text: `${a} km = ? m`, answer: a * 1000, helpText: "1 km sind 1000 m. Hänge drei Nullen an." }; signature = `len4-${a}`; }
        else if (type === 5) { const a = rand(1, 9) * 10; q = { text: <div className="text-center text-2xl sm:text-4xl font-bold">{a} cm + <span className="text-blue-500">? cm</span> = 1 m</div>, answer: 100 - a, helpText: "Denke daran: 1 m ist genau so lang wie 100 cm. Wie viel fehlt bis zur 100?" }; signature = `len5-${a}`; }
        else { 
          const m = rand(1, 9); const cm = rand(1, 99);
          q = { text: `${m} m ${cm} cm = ? cm`, answer: m * 100 + cm, helpText: `Verwandle zuerst die Meter in Zentimeter (${m} m = ${m*100} cm). Zähle dann die restlichen ${cm} cm dazu.` };
          signature = `len6-${m}-${cm}`;
        }
        ...
        else if (type === 2) { const a = rand(2, 15); q = { text: `${a * 1000} g = ? kg`, answer: a, helpText: "Streiche drei Nullen weg, denn 1000 g sind 1 kg." }; signature = `wei2-${a}`; }
        else if (type === 3) { const a = rand(1, 9) * 100; q = { text: <div className="text-center text-2xl sm:text-4xl font-bold">{a} g + <span className="text-blue-500">? g</span> = 1 kg</div>, answer: 1000 - a, helpText: "Denke daran: 1 kg ist genau so schwer wie 1000 g. Wie viel fehlt bis zur 1000?" }; signature = `wei3-${a}`; }
        else {
        ...
        const problems = [
          { text: "Ein Brett ist 2 m lang. Es werden 30 cm abgesägt. Wie lang ist es noch in cm?", ans: 170 },
          { text: "Tom wandert an 3 Tagen. Tag 1: 12 km, Tag 2: 9 km, Tag 3: 11 km. Wie lang ist die Strecke insgesamt in km?", ans: 32 },
          { text: "In eine Tüte passen 250 g Äpfel. Wie viel Gramm Äpfel sind in 4 Tüten?", ans: 1000 },
          { text: "Ein Bäcker braucht 2 kg Mehl. Er hat schon 800 g. Wie viele Gramm (g) fehlen ihm noch?", ans: 1200 },
          { text: "Ein Schwimmbecken ist 25 m lang. Lisa schwimmt 4 Bahnen. Wie viele Meter schwimmt sie insgesamt?", ans: 100 },
          { text: "Ein LKW darf höchstens 7000 kg wiegen. Er wiegt leer 3000 kg. Wie viele Tonnen (t) darf er noch laden? (Tipp: 1000 kg = 1 t)", ans: 4 }
        ];
        ...
        const cells: GridCell[] = [
          { id: "s0", val: base1, isGiven: true }, { id: "s1", val: base2, isGiven: true }, { id: "s2", val: base3, isGiven: true },
          { id: "s3", val: base1 + base2, isGiven: false }, { id: "s4", val: base2 + base3, isGiven: false },
  ...
  const finishRound = (finalScore: number, finalTimes: number[]) => {
    ...
    const newEntry: HistoryEntry = {
      ...
      mode: activeCategory === "mixed" ? "Gemischt" : currentModeInfo.name,
      score: finalScore,
  ...
  const startGame = (target: GameMode | Category) => {
    ...
    if (target === "calc" || target === "space" || target === "money" || target === "grade3" || target === "mixed") {
  ...
  const clearHistory = () => {
      if (window.confirm("Möchtest du den gesamten Verlauf löschen?")) {
          setHistory([]);
      }
  };
  ...
  const checkAnswer = () => {
    ...
    if (mode === "shopping" && currentQuestion.shoppingData) {
      ...
      const uA = parseMoney(multiInputs["money-comp-a"]); const uB = parseMoney(multiInputs["money-comp-b"]); const uDiff = parseMoney(multiInputs["money-comp-diff"]);
      ...
      const userQ = parseInt(multiInputs["div-quotient"] || "0"); const userR = parseInt(multiInputs["div-remainder"] || "0");
      ...
      const rA = parseInt(multiInputs["est-roundA"] || "0"); const rB = parseInt(multiInputs["est-roundB"] || "0"); const res = parseInt(multiInputs["est-res"] || "0");
      ...
      const i = multiInputs; const stepData = currentQuestion.stepData;
      ...
      stepData.steps.forEach((s, idx) => {
        ...
        if (idx > 0) {
          const prevRes = stepData.steps[idx - 1].res; const userStart = parseInt(i[`step-${idx}-start`] || "0");
      ...
      if (input !== "") isCorrect = parseMoney(input) === currentQuestion.answer;
    } else {
  ...
  const jumpToNextField = () => {
    ...
    else if (mode === "money_compare") allIds = ["money-comp-a", "money-comp-b", "money-comp-diff"];
    else if (currentQuestion?.stepData) {
      currentQuestion.stepData.steps.forEach((_, idx) => {
        if (idx > 0) allIds.push(`step-${idx}-start`);
        allIds.push(`step-${idx}-add`); allIds.push(`step-${idx}-res`);
      });
      allIds.push("main-result");
    } else if (currentQuestion?.gridData) {
      allIds = currentQuestion.gridData.cells.filter((c) => !c.isGiven).map((c) => c.id);
    }
      ...
      <div key={id} className="flex flex-col items-center">
        ...
        <div className={`flex items-center justify-center font-bold text-lg sm:text-2xl transition-all relative shadow-sm ${baseStyle} ${colorStyle}`}>
          {isGiven ? (<span>{value}</span>) : (
            <input type="text" inputMode="none" value={userInput} onChange={(e) => handleInputChange(id, e.target.value)} onFocus={() => { if (feedback === "none") setActiveCellId(id); }} className="w-full h-full text-center bg-transparent outline-none p-0 m-0 cursor-pointer" autoComplete="off" />
          )}
          {isWrong && (
            <div className="text-[10px] text-green-600 font-bold absolute -bottom-4 bg-white/80 px-1 rounded shadow-sm z-20 whitespace-nowrap">
              {mode === "money_compare" ? (moneyVal / 100).toFixed(2).replace(".", ",") : value}
            </div>
          )}
        </div>
      </div>
    ...
    return (
      <div className="flex flex-col items-center gap-4 mt-4">
        <div className="flex items-center gap-2 text-2xl font-bold text-slate-700">
          <span>{d.dividend} : {d.divisor} =</span>
          {renderMultiInputCell("div-quotient", d.quotient, false, "Ergebnis", "w-16 h-16 border-2 rounded-xl")}
          <span>R</span>
          {renderMultiInputCell("div-remainder", d.remainder, false, "Rest", "w-16 h-16 border-2 rounded-xl")}
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="flex flex-col items-center gap-4 mt-6">
        <div className="flex items-center gap-2 text-2xl font-bold text-slate-700 bg-blue-50 p-4 rounded-xl border border-blue-100">
          <span className="text-blue-500">{d.a}</span><span>·</span><span className="text-blue-500">{d.b}</span><span>+</span><span className="text-orange-500">{d.add}</span><span>=</span>
          {renderMultiInputCell("inv-res", d.result, false, undefined, "w-20 h-16 border-2 rounded-xl bg-white")}
        </div>
  ...
  const renderEstimation = () => {
    ...
    return (
      <div className="flex flex-col items-center gap-4 mt-4">
        <div className="text-lg font-bold text-slate-400 mb-2">{d.a} {d.op} {d.b}</div>
        <div className="flex items-center gap-2">
          {renderMultiInputCell("est-roundA", d.roundedA, false, "Rund 1", "w-20 h-14 border-2 rounded-lg")}
          <span className="text-2xl font-bold">{d.op}</span>
          {renderMultiInputCell("est-roundB", d.roundedB, false, "Rund 2", "w-20 h-14 border-2 rounded-lg")}
          <span className="text-2xl font-bold">=</span>
          {renderMultiInputCell("est-res", d.estResult, false, "Ergebnis", "w-20 h-14 border-2 rounded-lg bg-purple-50")}
        </div>
      </div>
    );
  };
    ...
    if (type === "cube_plan") {
        return (
            <div className="flex flex-col items-center gap-2 w-full">
                ...
                <div className="grid gap-1 bg-slate-200 p-2 rounded-xl shadow-inner mt-2" style={{ gridTemplateColumns: `repeat(${meta.cols}, 1fr)` }}>
                  {cells.map(c => renderMultiInputCell(c.id, c.val as number, false, undefined, "w-12 h-12 sm:w-16 sm:h-16 border-2 rounded-lg bg-white text-2xl"))}
                </div>
    ...
    if (type === "pyramid")
      return (
        <div className="flex flex-col items-center gap-2 mt-4">
          <div className="flex justify-center">{renderMultiInputCell(cells[5].id, cells[5].val as number, cells[5].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}</div>
          <div className="flex justify-center gap-1">{renderMultiInputCell(cells[3].id, cells[3].val as number, cells[3].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}{renderMultiInputCell(cells[4].id, cells[4].val as number, cells[4].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}</div>
          <div className="flex justify-center gap-1">{renderMultiInputCell(cells[0].id, cells[0].val as number, cells[0].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}{renderMultiInputCell(cells[1].id, cells[1].val as number, cells[1].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}{renderMultiInputCell(cells[2].id, cells[2].val as number, cells[2].isGiven, undefined, "w-16 h-10 sm:w-20 sm:h-14 border-2 rounded-lg mb-1")}</div>
        </div>
      );
    return null;
  };
  ...
  const renderGameContent = () => {
    ...
    return (
      <div className={`w-full flex-1 bg-white rounded-3xl shadow-lg p-4 flex flex-col items-center justify-center mb-6 lg:mb-0 relative overflow-y-auto transition-colors ${feedback === "correct" ? "bg-green-50 ring-4 ring-green-200" : feedback === "wrong" ? "bg-red-50 ring-4 ring-red-200" : ""}`}>
        ...
        {mode === "division_remainder" ? (renderDivisionRemainder()) 
        : mode === "inverse_calc" ? (renderInverseCalc()) 
        : mode === "estimation" ? (renderEstimation()) 
        : mode === "money_pay" ? (renderMoneyPay()) 
            ...
            <div className="flex flex-col items-center bg-white p-2 border-2 border-slate-200 shadow-inner rounded-xl overflow-hidden mt-4">
              <div className="grid grid-cols-5 gap-0 bg-slate-100 border-l-2 border-t-2 border-slate-300">
                ...
                <div className="w-14 h-12 border-b-2 border-r-2 border-slate-300 flex items-center justify-center text-xl font-mono cursor-pointer bg-white">
                  {renderMultiInputCell("main-result", currentQuestion.stepData.result, false, undefined, "w-full h-full border-none rounded-none")}
                </div>
                <div className="col-span-5 h-1 bg-slate-800 my-0"></div>
                {currentQuestion.stepData.steps.map((step, idx) => {
                  const isFirstRow = idx === 0;
                  const prevRes = isFirstRow ? currentQuestion.stepData?.start : currentQuestion.stepData?.steps[idx - 1].res;
                  return (
  ...
  const renderNumPad = () => (
    <div className="grid grid-cols-3 gap-3 h-64 shrink-0 mt-2">
      ...
      {currentQuestion?.isDecimal ? (
        ...
        <button onClick={mode === "money_compare" || mode.includes("gap") ? jumpToNextField : checkAnswer} className="bg-green-500 text-white text-2xl font-bold rounded-2xl shadow border-b-4 border-green-700 active:border-t-4 active:border-b-0">OK</button>
      )}
      {currentQuestion?.isDecimal && feedback === "none" && (
        <button onClick={mode === "money_compare" ? jumpToNextField : checkAnswer} className="col-span-3 bg-green-500 text-white font-bold rounded-2xl shadow border-b-4 border-green-700 py-2 mt-1">OK</button>
      )}
    </div>
  );
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 font-sans safe-area-inset">
        <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-3xl text-center relative overflow-hidden">
          ...
          <div className="absolute top-4 right-4 flex gap-2 z-20">
            <button onClick={() => setShowSettingsModal(true)} className="bg-slate-100 p-2 rounded-full shadow text-slate-500 hover:text-slate-700 transition-colors" title="Einstellungen">
              <Settings size={20} />
            </button>
            <button onClick={() => setMode("history")} className="bg-slate-100 p-2 rounded-full shadow text-blue-500 hover:text-blue-700 transition-colors" title="Auswertung & Verlauf">
              <History size={20} />
            </button>
            <button onClick={() => setSoundEnabled(!soundEnabled)} className="bg-slate-100 p-2 rounded-full shadow text-slate-400 hover:text-slate-600 transition-colors" title="Ton umschalten">
          ...
          {isOffline ? (
            offlineReady ? ( <div className="mb-4 rounded-xl bg-green-100 border border-green-300 text-green-900 px-4 py-2 text-sm font-bold text-center">✅ Offline verfügbar</div> ) : ( <div className="mb-4 rounded-xl bg-red-100 border border-red-300 text-red-900 px-4 py-2 text-sm font-bold text-center">❌ Noch nicht offline bereit</div> )
          ) : null}

          <div className="overflow-y-auto px-1 max-h-[70vh]">
            <CategorySection title="Klasse 3: Raum & Größen" cat="grade3" modes={["cube_buildings", "geometry_shapes", "units_length", "units_weight", "word_problem_units"]} onStart={startGame} />
              ...
              <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden text-left">
                <div className="p-4 bg-slate-100 flex justify-between items-center border-b border-slate-200">
                  <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <Settings className="text-slate-500" /> Einstellungen
                  </h2>
                  <button onClick={() => setShowSettingsModal(false)} className="bg-slate-200 p-1.5 rounded-full text-slate-600 hover:bg-slate-300">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3].map(level => (
                        <button
                          key={`diff-${level}`}
                          onClick={() => setSettings({ ...settings, difficulty: level as 1|2|3 })}
                          className={`flex-1 py-2 rounded-xl font-bold border-2 transition-colors ${settings.difficulty === level ? 'bg-blue-500 text-white border-blue-600' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'}`}
                        >
                          {level === 1 ? "Einfach" : level === 2 ? "Normal" : "Schwer"}
                        </button>
                      ))}
                    </div>
                  </div>

                  {/* Baupläne Rastergröße */}
                  <div>
                    <h3 className="font-bold text-slate-700 mb-2">Baupläne Rastergröße</h3>
                    <div className="flex flex-wrap gap-2">
                      {["random", "2x4", "3x3", "3x4", "4x4"].map(size => (
                        <button
                          key={`grid-${size}`}
                          onClick={() => setSettings({ ...settings, cubeGridSize: size as AppSettings["cubeGridSize"] })}
                          className={`px-4 py-2 rounded-xl font-bold border-2 transition-colors ${settings.cubeGridSize === size ? 'bg-orange-500 text-white border-orange-600' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'}`}
                        >
                          {size === "random" ? "Zufall" : size}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Aktive Körper */}
                  <div>
                    <h3 className="font-bold text-slate-700 mb-2">Geometrische Körper</h3>
                    <div className="grid grid-cols-2 gap-2">
                      {[{id: "cube", name: "Würfel"}, {id: "cuboid", name: "Quader"}, {id: "cylinder", name: "Zylinder"}, {id: "pyramid", name: "Pyramide"}, {id: "sphere", name: "Kugel"}, {id: "cone", name: "Kegel"}, {id: "prism", name: "Dreiecksprisma"}].map(shape => (
                        <label key={shape.id} className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100">
                          <input
                            type="checkbox"
                            checked={settings.activeShapes.includes(shape.id)}
                            onChange={(e) => {
                              let newShapes = [...settings.activeShapes];
                              if (e.target.checked) newShapes.push(shape.id);
                              else newShapes = newShapes.filter(s => s !== shape.id);
                              // Verhindere, dass alles abgewählt wird
                              if (newShapes.length === 0) newShapes = ["cube"];
                              setSettings({ ...settings, activeShapes: newShapes });
                            }}
                            className="w-5 h-5 accent-indigo-500"
                          />
                          <span className="font-medium text-slate-700 text-sm">{shape.name}</span>
                        </label>
                      ))}
                    </div>
                  </div>
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col p-4 font-sans safe-area-inset">
        <div className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-3xl mx-auto flex flex-col h-full">
          <div className="flex justify-between items-center mb-6">
            <button onClick={() => setMode("menu")} className="bg-slate-100 p-3 rounded-xl shadow text-slate-600 hover:bg-slate-200 transition-colors flex items-center gap-2 font-bold">
              <Home size={20} /> Zurück
            </button>
            <h2 className="text-2xl font-extrabold text-slate-800 flex items-center gap-2">
              <History className="text-blue-500" /> Verlauf
            </h2>
            <button onClick={clearHistory} className="bg-red-50 p-3 rounded-xl border border-red-200 text-red-500 hover:bg-red-100 transition-colors flex items-center gap-2 font-bold text-sm">
          ...
          <div className="flex-1 overflow-y-auto pr-2">
            {history.length === 0 ? (
              ...
              <div className="grid gap-3">
                {history.map((entry) => (
                  <div key={entry.id} className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl text-white ${entry.score === entry.total ? 'bg-green-500' : entry.score > entry.total / 2 ? 'bg-yellow-500' : 'bg-red-500'}`}>
                        {entry.score}/{entry.total}
                      </div>
                      <div>
                        <div className="font-bold text-slate-800 text-lg">{entry.mode}</div>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs font-medium text-slate-500 mt-1">
                          <span className="flex items-center gap-1"><CalendarDays size={14} /> {entry.timestamp}</span>
                          <span className="flex items-center gap-1"><Clock size={14} /> Ø {entry.avgTime}s pro Aufgabe</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  if (isRoundOver) {
    ...
    const isPerfect = correctCount === QUESTIONS_PER_ROUND;
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md text-center relative z-10">
          ...
          <h2 className="text-3xl font-bold mb-2">{percentage === 100 ? "Perfekt!" : "Gut gemacht!"}</h2>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col p-4 font-sans safe-area-inset overflow-hidden">
      <div className="flex justify-between items-center mb-4 relative z-20">
        <button onClick={() => setMode("menu")} className="bg-white p-3 rounded-xl shadow text-slate-600 hover:bg-slate-50 transition-colors">
          <Home className="w-6 h-6" />
        </button>
        <div className="flex flex-col items-center">
          <div className={`px-3 py-1 rounded-full text-xs font-bold text-white mb-1 ${currentModeInfo.color}`}>{currentModeInfo.name}</div>
          <div className="text-2xl font-black text-slate-700">{questionIndex} <span className="text-lg text-slate-400">/ {QUESTIONS_PER_ROUND}</span></div>
        </div>
        <div className="flex gap-2">
          {/* NEU: HILFE BUTTON */}
          <button onClick={() => setShowHelpModal(!showHelpModal)} className={`bg-white p-2 rounded-xl shadow transition-colors flex items-center justify-center ${showHelpModal ? 'text-blue-500 bg-blue-50 ring-2 ring-blue-300' : 'text-slate-400 hover:text-blue-400'}`}>
            <HelpCircle className="w-6 h-6" />
          </button>
          <div className="bg-white px-3 py-2 rounded-xl shadow flex items-center gap-2">
            <Star className="w-5 h-5 text-yellow-400" fill="currentColor" />
            <span className="font-bold text-slate-700 text-xl">{correctCount}</span>
          </div>
        </div>
      </div>
